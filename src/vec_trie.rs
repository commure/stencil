...
/// A trie with the following properties, chosen for its use in `stencil`:
///
...
/// - You can only navigate the trie down and foward, not up or backward, because `stencil` does
///   not need to.
/// - It does not use generational indices, so the ABA problem is present. But `stencil` is the
